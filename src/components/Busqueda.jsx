...
const Busqueda = () => {
  ...
  return (
    ...
    >
      <Dialog
        as="div"
        className="fixed inset-0 z-10 mt-20 overflow-y-auto p-4 sm:p-20 md:p-20"
              ...
              >
                {proyectosFiltrados.map((proyecto) => (
                    <Combobox.Option
                        key={proyecto._id}
                        value={proyecto}
                        className={({ active }) => classNames('cursor-default select-none relative py-2 pl-10 pr-4', active ? 'bg-sky-600 text-white' : 'text-gray-900')}
                    >
                        {proyecto.nombre}
                    </Combobox.Option>
                ))}
